...
function setTab(tab) {
  ...
  ["stories", "matrix", "check", "runs"].forEach((t) => {
        ...
        ${list
          ...
          <tr>
            ...
            <td class="muted">${(us.acceptance_criteria || [])
              ...
              .join("<br>") || "—"}</td>
            <td><button class="btn btn-danger" data-del-us="${us.id}">Delete</button></td>
          </tr>`
          )
          .join("")}
      </tbody>
      ...
      <tbody>
        ${list
          ...
          <tr>
            ...
            <td><button class="btn btn-danger" data-del-pm="${r.id}">Delete</button></td>
          </tr>`
          )
          .join("")}
      </tbody>
...
/* ---------- Custom dropdown (tab Run analysis) ---------- */
...
async function loadProjectRuns() {
  ...
  try {
    ...
    el.innerHTML = `
      <table class="table">
        <thead><tr><th>Status</th><th>User story</th><th>When</th></tr></thead>
        <tbody>
          ${data.items
            .map(
              (r) => `
...
async function loadGlobalHistory() {
  ...
  try {
    ...
    el.innerHTML = `
      <table class="table">
        <thead><tr><th>Status</th><th>Types</th><th>User story</th><th>When</th></tr></thead>
        <tbody>
          ${data.items
            .map(
              (r) => `
